...
const renderIndicators = (communityViewDto, profileType) => React.createElement(View, { style: styles.indicatorsContainer }, INDICATORSTRATEGY[profileType].map(code => {
    if (communityViewDto[code].length > 1) {
        return React.createElement(Image, { key: code, style: styles.roleIndicator, resizeMode: 'contain', source: require('Musl/images/profile/safety/2.png') });
    }
    return React.createElement(Image, { key: code, style: styles.roleIndicator, resizeMode: 'contain', source: ICONS[code.toUpperCase() + '_ICONS'][communityViewDto[code][0]] });
}));
export default class CommunityGridItem extends PureComponent {
    constructor() {
        super(...arguments);
        this.onItemClick = () => {
    ...
    render() {
        ...
        const visitedIcon = (React.createElement(Image, { style: styles.visitStatus, source: require('Musl/images/profile/icon-visited.png') }));
        ...
        const nickname = nicknameFull.length > 10 ? nicknameFull.substring(0, 9)
            .trim() + '...' : nicknameFull;
        return React.createElement(TouchableOpacity, { style: styles.itemContainer, onPress: this.onItemClick },
            Platform.OS === 'android' ?
                React.createElement(FastImage, { style: styles.userPic, source: {
                        uri: sourceUrl,
                        priority: FastImage.priority.normal,
                        headers: { Cookie: 'SESSION=' + this.props.sessionId }
                    }, resizeMode: FastImage.resizeMode.contain }) :
                React.createElement(Image, { style: styles.userPic, source: {
                        uri: sourceUrl,
                        headers: { Cookie: 'SESSION=' + this.props.sessionId },
                        cache: 'force-cache'
                    }, resizeMode: 'contain' }),
            React.createElement(ImageBackground, { style: styles.top, resizeMode: 'stretch', source: require('Musl/images/global/shadow-bg.png') },
                React.createElement(UiBlockVerticalCenter, null,
                    React.createElement(UiBlockHorizontalEdges, null,
                        React.createElement(UiBlockHorizontal, null,
                            React.createElement(Text, { style: styles.userName, ellipsizeMode: 'tail', numberOfLines: 1 }, nickname))))),
            React.createElement(Image, { style: styles.activeStatus, source: STATUS_ICONS[model.status] }),
            model.visited ? visitedIcon : null,
            React.createElement(View, { style: styles.bottom },
                React.createElement(UiBlockVerticalCenter, null,
                    React.createElement(UiBlockHorizontalEdges, null,
                        React.createElement(Text, { style: styles.distance, numberOfLines: 1 }, model.distanceToDisplay),
                        indicators))));
    }
}
const resizeItem = (dWidth, colCount, colSpacing) => {
...
//# sourceMappingURL=community-grid-item.js.map